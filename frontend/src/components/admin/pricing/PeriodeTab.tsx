...
export default function PeriodeTab() {
  ...
  const fetchPeriode = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/periode`, {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEditMode 
        ? `${process.env.NEXT_PUBLIC_API_URL}/api/admin/periode/${formData.id_periode}`
        : `${process.env.NEXT_PUBLIC_API_URL}/api/admin/periode`;
  ...
  const handleDelete = async () => {
    ...
    setErrorMessage(null);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/periode/${periodeToDelete.id_periode}`, {
        method: "DELETE",
        headers: { "X-API-Key": process.env.NEXT_PUBLIC_X_API_KEY || "" },
      });
      if (res.ok) {
