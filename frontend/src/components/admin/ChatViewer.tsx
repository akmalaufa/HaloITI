...
export default function ChatViewer({ initialLeads }: { initialLeads: Lead[] }) {
  ...
  useEffect(() => {
    ...
    const fetchChats = async () => {
      setIsLoading(true);
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/chat-logs/${selectedLead}?limit=20&offset=0`, {
          headers: {
  ...
  useEffect(() => {
    ...
    const pollNewChats = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/chat-logs/${selectedLead}?limit=10&offset=0`, {
          headers: { "X-API-Key": process.env.NEXT_PUBLIC_X_API_KEY || "" }
        });
        if (!res.ok) return;
        const json = await res.json();
  ...
  const fetchOlderHistory = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/chat-logs/${selectedLead}?limit=20&offset=${offset}`, {
        headers: { "X-API-Key": process.env.NEXT_PUBLIC_X_API_KEY || "" }
      });
      if (!res.ok) throw new Error("Gagal mengambil riwayat chat");
